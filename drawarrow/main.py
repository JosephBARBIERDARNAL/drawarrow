...
def fig_arrow(
    ...
    inflection_position: list[float] | tuple[float] | None = None,
    ...
    **kwargs,
    ...
    """
    Draw an arrow on a Matplotlib figure using a `FancyArrowPatch`. The coordinate
    system used is that of a Matplotlib Figure (from 0 to 1).

    This function is very similar to what [`ax_arrow()`](ax_arrow.md) does: only
    the change of coordinate system and the `fig/ax` arguments. You can find out
    more about how coordinate systems work in Matplotlib
    [here](https://python-graph-gallery.com/589-how-to-change-coordinate-system/).
...
def ax_arrow(
    ...
    **kwargs,
    ...
    """
    Draw an arrow on a Matplotlib axes using a `FancyArrowPatch`. The coordinate system
    used is that of a Matplotlib axes (data coordinates).

    This function is very similar to what [`fig_arrow()`](fig_arrow.md) does: only the
    change of coordinate system and the `fig/ax` arguments. You can find out more about
    ...
    Args:
        tail_position: Position of the tail of the arrow
        head_position: Position of the head of the arrow
        inflection_position: Current behavior may be unexpected and will probably change in
            the future. Optional position of the inflection point
        double_headed: Whether the arrow has two heads or not
        fill_head: Whether the arrowhead is filled or not
        invert: Whether to invert or not the angle of the arrow (only used if `radius`!=0)
        radius: Rounding radius of the edge. If `inflection_position` is not None, then
